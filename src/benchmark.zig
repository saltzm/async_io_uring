...
pub fn benchmark(ring: *AsyncIOUring) !void {
    ...
    const cqe_connect = try ring.connect(NoUserData, client, &address.any, address.getOsSockLen());
    ...
    while (num_ops < max_ops) : (num_ops += 1) {
        ...
        const send_result = try ring.send(NoUserData, client, buffer_to_send[0..num_bytes_to_send], @intCast(u32, num_bytes_to_send));
        assert(send_result.res == num_bytes_to_send);

        // Receive the response.
        const cqe_recv = try ring.recv(NoUserData, client, input_buffer[0..], 0);
